...
func (c *service) Build(ctx context.Context, req buildRequest) (res map[string]interface{}, err error) {
	...
	fmt.Println("姜猛修改标记，进入Build接口   +++ 打印结束")
	jmstr := fmt.Sprintln("%d", req.GroupID)
	fmt.Println("姜猛修改标记，gtoupid是" + jmstr)
	var project []*types.Project

	if req.GroupID > 0 {
		jmstr := fmt.Sprintln("姜猛修改标记，groupid大于0，gtoupid是"+"%d", req.GroupID)
		fmt.Println(jmstr)
		if isAdmin == false {
			//非超管，需要验证组权限
		...
		jmstr := fmt.Sprintln("姜猛修改标记，groupidb并不大于0，gtoupid是"+"%d", req.GroupID)
		fmt.Println(jmstr)
		if isAdmin == false {
			//获取权限内的项目
			...
			fmt.Println("根据namespace的值", req.Namespace, "进行查询project表")
			fmt.Println("姜猛修改标记，查到的project是", project, "  打印结束")
			if err != nil {
	...
	if len(project) > 0 {
		fmt.Println("姜猛修改标记，project列表的数量大于0，开始进去循环")
		for _, v := range project {
			projectNames = append(projectNames, v.Name)
			fmt.Println("姜猛修改标记，循环内peojectNames为", projectNames)
		}
	}
	...
	fmt.Println("姜猛修改标记，Build.CountStatistics返回的count数值是", count)
	...
	fmt.Println("姜猛修改标记，Build.FindStatisticsOffsetLimit返回的结果list的值为", list)
	if err != nil {
		fmt.Println("姜猛修改标记，list为0这里出错了")
	...
	if err != nil {
		fmt.Println("姜猛修改标记，第三次groupby错误")

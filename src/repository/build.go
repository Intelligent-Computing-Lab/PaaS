...
func (c *build) FindStatisticsOffsetLimit(req StatisticsRequest, offset, limit int) (res []*types.Build, err error) {
	...
	if req.Namespace != "" {
		...
		fmt.Println("姜猛修改标记，在builds表中根据ns查到的query值为", query)
	}
	if req.Name != "" {
	...
	err = query.Order("id desc").Offset(offset).Limit(limit).Find(&res).Error
	fmt.Println("姜猛修改标记，返回前builds表查询结果经过分组以及orderby后的结果是否有错误", err)
	return
}
...
func (c *build) GetGroupByBuilds(req StatisticsRequest, groupBy string) (ress []Ress, err error) {
	...
	fmt.Println("姜猛修改标记，看看新groupby之前有没有错误", query)
	if req.Namespace != "" {
		query = query.Where("namespace = ?", req.Namespace)
	}
	if req.Name != "" {
	...
	//}
	fmt.Println("姜猛修改标记，Find前看看query", query)
	//姜猛修改标记删除了下面的函数
	//err = query.Find(&ress).Error
	fmt.Println("姜猛修改标记，返回前看看err", err)
	return
}
